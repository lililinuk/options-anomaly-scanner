...
export const fieldGlossary = {
  ...
  persistent_positioning_score: entry("持續部位分數", "Persistent Positioning Score", "跨多個有效 OI observations 是否持續形成或下降。", "各自計算 3／5／10 個有效 observation windows，採可用視窗最高分，並顯示視窗第一／最後 vendor OI 日期與有效筆數。", "分數描述 OI positioning 的持續性；PERSISTENT_BUILD／DECLINE 都不是交易方向。", "有效 observations 不等於連續日曆日；目前觸發 freshness 未校準時標為 CALIBRATION_REQUIRED。", "每日 vendor OI observation date"),
  ...
  discovery_evidence_breadth: entry("歷史證據廣度", "Legacy Evidence Breadth (v1.2)", "僅保留於歷史 v1.2 evidence；Phase 2A vNext 已移除 Evidence Breadth／MULTI_EVIDENCE。", "不在 vNext 計算。", "只供歷史診斷，不參與目前 Product Candidate qualification。"),
  ...
  expiry_activity: entry("到期日活動", "Expiry Activity", "目前 expiry 層級的 activity concentration，是 Phase 2A vNext Core Discovery；0DTE 是其中的特殊歷史校準方法。", "非 0DTE 沿用可比 expiry 活動規格；0DTE 使用同 ticker 歷史基準。", "回答目前活動是否集中於此 expiry。", "不與 Radar 或 Contract Persistence 分數平均。", "每次活動快照或互動掃描"),
  ...
  trigger_sources: entry("觸發來源", "Trigger Sources", "Phase 2A vNext 只列出 RADAR_EVENT、EXPIRY_ACTIVITY、CONTRACT_PERSISTENCE；Expiry Persistence 與 Structural Cold Start 已退出 active discovery。", "所有合格 contract／expiry anomalies 依 ticker 分組到非持久化 Product Candidate projection。", "同一 ticker 可保留多筆 anomalies。", "不合併成 universal、ticker 或 conviction score。"),
  ...
  dte: entry("距到期日天數", "DTE", "到期日相對明確證據錨點日期的日曆天數。", "Expiry Activity 使用經 XNYS 驗證的 America/New_York session date；Radar 使用 vendor observation date；archive Contract Persistence 使用 vendor OI date。", "每個 API 值同時顯示 dte_anchor_date 與 dte_anchor_type，不以較晚 local scan date 重解讀 vendor evidence。"),
  ...
  neighbor_ratio: entry("可比鄰近到期日成交量比率", "Comparable Neighbor Ratio", "非 0DTE Expiry Activity scoring 實際使用的目前 expiry volume 相對可比非零 DTE peers 中位數。", "current expiry volume / median comparable peer volume。", "peer count／quality／median／DTEs 分開顯示；比較器不足時為 NULL。", "0DTE 不使用此比較器；其 raw cross-expiry ratio 僅為 descriptive。"),
