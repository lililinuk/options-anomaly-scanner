  ...
  return (
    <>
      <section className="scan-strip" aria-label="Latest scan status">
        ...
        <div><span>Source reuse</span><strong>{data.scan ? `${data.scan.cache_hits} cached · ${data.scan.fresh_requests} fresh` : "—"}</strong></div>
      ...
      <section className="panel results-panel">
        <div className="panel-header"><div><span className="eyebrow">Same-day positioning</span><h2>Phase 2A Results</h2></div><small>Activity structure only · not a trade recommendation</small></div>
        <div className="table-wrap results-table"><table><thead><tr>{visibleAnalyticalColumns.map((key) => <th key={key} title={fieldGlossary[key].定義}>{fieldGlossary[key].englishField}</th>)}</tr></thead>
          <tbody>{data.results.length ? data.results.map((row) => <tr key={String(row.ticker)}>{visibleAnalyticalColumns.map((key) => <td key={key}>{display(row[key])}</td>)}</tr>) : <tr><td colSpan={visibleAnalyticalColumns.length}><div className="empty-state"><div className="radar" aria-hidden="true"><span /></div><h3>No Phase 2A scan yet</h3><p>Run the manual MAG7 scan to persist evidence-backed positioning results.</p></div></td></tr>}</tbody>
        </table></div>
      </section>
    </>
  );
