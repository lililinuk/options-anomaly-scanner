...
**Date:** 2026-08-17
**Status:** `v0.1 — FOUNDER APPROVED 2026-08-17`
**Approval record:** Founder approved v0.1 on 2026-08-17 with: (a) D8 finalized as **Trend State = OPTIONAL derived display**; (b) addition of the **Live / Research Separation — Non-Negotiable** section (§8.1, verbatim founder text); (c) all PART 12 checklist items approved.
**Authorization state:** This document is now the **working architecture baseline**. Approval of the spec itself authorizes no code change, no DB write, and no paid Nightwatch call. The next authorized action is **Stage 1 — Read-Only Repository Proof Gate** (0 writes · 0 paid calls · 0 code changes). Every subsequent stage requires its own scoped execution package.
...
8. Founder approval additions of 2026-08-17: D8 finalization + §8.1 Live / Research Separation — Non-Negotiable (founder-authored)
...
| D8 | Trend State / Adjacent Expiry Context | **OPTIONAL** — founder confirmed on approval: **Trend State = OPTIONAL derived display** | §5.2 / §5.4: both are optional secondary display. Trend State is a derived display value (computed from stored close/SMA20/SMA50 at render time; semantics unchanged). Underlying raw fields (close, SMA20/50; adjacent-expiry raw cells) remain persisted. |
...
live runtime never consumes Forward Outcome or any future-derived
feature (§8.1 Live / Research Separation — Non-Negotiable)
```
...
OPTIONAL  20-session high/low · Trend State (D8 final: OPTIONAL derived
          display — computed at render time from stored close/SMA20/SMA50;
          UPTREND/DOWNTREND/MIXED/UNKNOWN semantics unchanged)
```
...
## 8.1 Live / Research Separation — Non-Negotiable

*(Founder-authored text, added at approval 2026-08-17; red-line status equal to PART 1.)*

1. Live Scanner runtime MUST NEVER consume Forward Outcome,
   future T+N prices, MFE/MAE, or any feature derived from data
   unavailable at candidate decision time.
2. Forward Outcome belongs exclusively to the OFFLINE
   Research / Calibration Loop. It is used to evaluate historical
   Product Candidates and calibrate Actionability rules/models;
   it is never a live candidate input.
3. Live Phase 3 / Actionability consumes only:
   - current/as-of Phase 2A evidence,
   - frozen/as-of Phase 2B context,
   - a previously validated and versioned Actionability rule/model.
   It MUST NOT consume the Forward Outcome of the candidate
   currently being evaluated.
4. Every Actionability rule/model deployed to live runtime must
   carry an immutable version and calibration provenance.
5. Training/research features and live/serve features must have
   information-time parity. Any feature unavailable at live
   decision time is forbidden from calibration inputs.

## 8.2 Additional boundary rules
...
# PART 12 — Founder Review Checklist — APPROVED 2026-08-17

All items approved by the founder on 2026-08-17:

- [x] D1–D8 encodings (§0.2) — D8 finalized: Trend State = OPTIONAL derived display
- [x] Five-block Phase 2B composition with CORE / OPTIONAL / EXPANDED split (PART 5)
- [x] ProductCandidate + ProductCandidateTrigger as first-class persisted entities (§2.2)
- [x] Frozen First-Knowledge Research Snapshot semantics (§4.3)
- [x] Archive-only Dealer/GEX + removal of the dead heatmap call after Stage 1 proof (§5.4)
- [x] Consolidated defect plan and stage assignments (PART 9)
- [x] Stage plan order: 1 → 2 → 3 → (4A ∥ 4B) → 5 → 6 → 7 → 8 → 9 (PART 10)
- [x] Research-Loop / Live-Loop boundary statement (PART 8), extended at approval with §8.1 Live / Research Separation — Non-Negotiable
- [x] Do-Not-Change list (PART 11)

This spec is now the working architecture baseline (Stage 0 complete). Nothing is implemented until Stage 1 (read-only proof gate) has produced its evidence report; each subsequent stage requires its own scoped execution package.
